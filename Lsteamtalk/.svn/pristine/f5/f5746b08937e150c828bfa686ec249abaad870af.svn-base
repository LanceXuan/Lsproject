...
/** * @author  Lance  * @date 创建时间：2015年12月5日 下午3:27:24 * @version 1.0 * @parameter  * @since  * @return  */
...
public class FileUpLoadAction extends HttpServlet{
	...
	public void init(ServletConfig config) throws ServletException {
		...
		String wwe = servletCtx.getRealPath("/");
	...
	protected void doPost(HttpServletRequest request,
		...
		IMUser usr = service.findIMUser(username);
		if(usr !=null && usr.getId() >0 ){
			String salt = usr.getSalt();
			String newpwd = MD5Util.md5(pwd);
			if(usr.getPassword().equals(MD5Util.md5(newpwd+salt))){
				// 获得磁盘文件条目工厂
		...
		//		String username = request.getParameter("username") == null?"":request.getParameter("username");
				StringBuffer photourl =request.getRequestURL();
				
				System.out.println("绝对路径:"+photourl);
				System.out.println("dsfsd"+username);
				try {
					// 提交上来的信息都在这个list里面
					// 这意味着可以上传多个文件
					// 请自行组织代码
					...
					String namefile = df.format(new Date());
					namefile = namefile+SmsCodeUtil.createRandom(true, 4)+"."+prefix;
					photourl =photourl.append("/"+namefile);
					System.out.println("文件名dsf:" + photourl);
					// 真正写到磁盘上
					item.write(new File(PATH_FOLDER, namefile)); // 第三方提供的
					boolean isok = service.uodateavatar(username,photourl.toString());
					PrintWriter writer = response.getWriter();
					if(isok){
					writer.print("{");
					writer.print("msg:\"修改成功\"");
					writer.print(",photourl:\""+photourl.toString()+"\"");
					writer.print(",status:\"1\"");
					writer.print("}");
					}else{
						writer.print("{");
						writer.print("msg:\"保存失败\"");
						writer.print(",photourl:\"no\"");
